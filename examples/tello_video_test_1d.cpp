...
int main()
{
  ...
  */
  ...
  map.MapFloat(PITCH, padId, gainput::PadButtonLeftStickY);
  map.MapFloat(ROLL, padId, gainput::PadButtonLeftStickX);
  map.MapFloat(YAW, padId, gainput::PadButtonRightStickX);
  map.MapFloat(THROTTLE, padId, gainput::PadButtonLeftStickY);
  ...
  */
  ...
  std::thread t2([&](){
    ...
    while(!should_exit)
    {
      ...
      auto pitch = map.GetFloat(PITCH);
      auto roll = map.GetFloat(ROLL);
      auto yaw = map.GetFloat(YAW);
      auto throttle = map.GetFloat(THROTTLE);

      LOG(INFO) << "pitch: " << pitch<< ", " << roll<< ", " << yaw << ", "  << throttle;
